...
CS_COLS = [
    ...
    "cs_prod_indirect_labor", "cs_prod_indirect_others", "cs_direct_labor_costs",
    "cs_depreciation_others", "cs_mfg_overhead"
]
def upsert_cost_csv_to_sql(csv_path: str, db_path: str):
    ...
    rename = {
        ...
        "tariff_cost": "cs_tax_portion",
        "prod_indirect_labor": "cs_prod_indirect_labor",
        "prod_indirect_others": "cs_prod_indirect_others",
        "direct_labor_costs": "cs_direct_labor_costs",
        "depreciation_others": "cs_depreciation_others",
        "manufacturing_overhead": "cs_mfg_overhead",
    ...
    # Warning only sum_r チェック
    sum_r = df[CS_COLS].sum(axis=1)
    bad = df.loc[sum_r > 1.0 + 1e-9, ["product_name", "node_name"]]
    if not bad.empty:
        rows = bad.astype(str).agg(" / ".join, axis=1).tolist()
        print(f"[WARNING] sum_r > 1.0 rows (continuing anyway): {rows}", flush=True)
    # SQL 書き込み
    ...
    ON CONFLICT(node_name, product_name) DO UPDATE SET
      {', '.join([f'{col}=excluded.{col}' for col in CS_COLS])}
    ;
    """
    rows = []
    for _, r in df.iterrows():
        row = [r["node_name"], r["product_name"], 1]
        row.extend([float(r[c]) for c in CS_COLS])
