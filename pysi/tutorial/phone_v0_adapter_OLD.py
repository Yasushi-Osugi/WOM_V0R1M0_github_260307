...
def load_phone_v0(data_dir: str | Path) -> PhoneV0Model:
    data_dir = Path(data_dir)

    ts = pd.read_csv(data_dir / "phone_timeseries.csv")
    # normalize
    ...
    capacity = None
    if ((ts["node_name"] == "MOM_PHONE") & (ts["item"] == "capacity")).any():
        capacity = _series("MOM_PHONE", "capacity")

    # --- 追加：initial_inventory の読み取り（なければ 0） ---
    init_inv = 0.0
    if ((ts["node_name"] == "DAD_PHONE") & (ts["item"] == "initial_inventory")).any():
        # 月列は何でも良いが、基本は先頭月に1行だけ置く想定
        init_inv_rows = ts[(ts["node_name"] == "DAD_PHONE") & (ts["item"] == "initial_inventory")]
