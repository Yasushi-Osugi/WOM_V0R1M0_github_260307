...
def plot_pharma_v0(
    ...
    *,
    ...
    # --- CO / CO+D (total need) ---
    # NOTE:
    #   model.backlog is "end-of-month backlog_now" from the simulator.
    #   For capacity comparison, we need "start-of-month carryover (= backlog_prev)".
    #
    # carryover[t] = backlog[t-1]  (carryover at month t start)
    carryover = np.concatenate(([0.0], backlog[:-1])) if n >= 2 else np.asarray([0.0], dtype=float)
    co_plus_d = carryover + demand
    ...
    # CO: carryover at start-of-month (dark blue, square markers)
    ax.plot(
        x, carryover,
        color="darkblue",
        linewidth=2.2,
        marker="s",
        markersize=4.5,
        label="CO: Backlog / carryover (start-of-month)",
        zorder=7,
    )
    ...
    order = [
        ...
        "CO: Backlog / carryover (start-of-month)",
        "Import supply (to DC)",
        "Inventory (DC)",
