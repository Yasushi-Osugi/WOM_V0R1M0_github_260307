...
def inbound_MOM_leveling_vs_capacity(out_root, in_root, mom_name="MOM"):
    ...
    # ===== PATCH: env を拾う（in_root に無ければ out_root からも拾う） =====
    env = getattr(in_root, "_wom_env", None) or getattr(out_root, "_wom_env", None)

    for w in range(W):
        # ===== PATCH: 週次cap（env.weekly_capability[mom_name][w]）があればそれを優先 =====
        cap_w = int((getattr(env, "weekly_capability", {}) or {}).get(mom_name, [cap] * W)[w]) if env else cap
