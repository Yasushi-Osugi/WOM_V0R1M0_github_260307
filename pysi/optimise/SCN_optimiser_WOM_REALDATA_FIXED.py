# SCN_optimiser_WOM_REALDATA_FIXED.py
# あなたの会社のPSI Planner実データ完全対応版（2025.11.20 最終修正）
...
for _, row in df_p.iterrows():
    p = row['product_name']
    node = row['node_name']
    if not str(node).startswith('CS_'): continue
    for m in range(1, 13):
        val = row[f'm{m}']
        if val == 0: continue
        weekly = val / 4.345
...
existing_flow = {(i,j): 1e9 for i,j in A}  # 輸送は当面無制限
