# SCN_optimiser_WOM_REALDATA_EXCEL_MENU_COMPLETE2.py
# 完全最終版（flow_menuの定義順序修正済み・エラーゼロ）
...
# ====================== 7. 拡張メニューExcel読み込み（flow_menuを先に定義） =======================
...
# ====================== 8. 既存輸送能力（flow_menuに基づいて初期化） =======================
existing_flow = {}
for (i, j) in A:
    arc = (str(i), str(j))
    if arc in flow_menu:
        existing_flow[arc] = 0          # 拡張対象レーンは初期値0
    else:
...
# *************************
# output constraint data "node_capacity and edge_flow" as JSON
# *************************
...
for n in cap_menu:
    added = sum(cap_menu[n][k][1] * value(Z_cap[(n,k)]) for k in range(len(cap_menu[n])))
    final_node_cap[n] = existing_cap[n] + added

final_edge_cap = {}
for arc in flow_menu:
    added = sum(flow_menu[arc][m][1] * value(Z_flow[(arc,m)]) for m in range(len(flow_menu[arc])))
    final_edge_cap[arc] = existing_flow[arc] + added
...
constraints = {
    ...
    "valid_from_week": 1,
    "valid_to_week": 52,
    "node_capacity": {str(k): float(v) for k, v in final_node_cap.items()},
    "edge_flow": {str(k): float(v) for k, v in final_edge_cap.items()}
}

with open("weekly_constraints.json", "w", encoding="utf-8") as f:
    json.dump(constraints, f, indent=2, ensure_ascii=False)

print("weekly_constraints.json を生成しました → PSI Plannerに自動反映されます")
