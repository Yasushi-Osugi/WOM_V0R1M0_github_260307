...
def plot_mom_psi_cap(frame, env, product: str, mom_name: str, root_outbound):
    ...
    cap = (((getattr(env, "weekly_capability", {}) or {}).get(product, {}) or {}).get(mom_name, None))
    if cap is None:
        tk.Label(frame, text=f"No weekly_capability for {product}:{mom_name}").pack()
        return

    W = len(cap)
    mom = find_node_by_name(root_outbound, mom_name)
    node = mom if mom is not None else root_outbound
    ...
    for w in range(W):
        try:
            s_series.append(count_lots(node.psi4supply[w][0]))
        except Exception:
            s_series.append(0)
        try:
            i_series.append(count_lots(node.psi4supply[w][2]))
        except Exception:
            i_series.append(0)
        try:
            p_series.append(count_lots(node.psi4supply[w][3]))
        except Exception:
            p_series.append(0)
    ...
    ax.set_title(f"{product} / {mom_name} : PSI vs Capability")
    ax.set_xlabel("Week")
    ax.set_ylabel("Lots")
...
class WOMCockpit(tk.Tk):
    ...
    def refresh(self):
        ...
        # plots
        plot_mom_psi_cap(self.frame_psi_plot, self.env, prod, mom, root_ot)
