...
#   CSV:
#     {data_dir}/node_geo.csv  columns: node_name(or name), lat, lon
# ------------------------------------------------------------
...
class WorldMapView:
    ...
    def set_selected_node(self, node_name: str) -> None:
        """
        外部（cockpit/network/tree）から選択ノードを反映してハイライトする。
        """
        if not node_name:
            return
        # 既存のハイライトを消す
        ...
        # node_name を中心に注釈・ハイライト（既存ロジック流用）
        ...
        x, y = pos[node_name]
        # 既存の「クリック時と同等の描画」を簡易に再現
        try:
            self._annotate_node(ax, node_name, x, y)  # ←もし既存にあるなら使う
        except Exception:
            # 最低限：点を強調してラベル
            try:
    ...
    def show(self) -> None:
        ...
        # [PATCH] pass data_crs into _auto_fit so set_extent uses lon/lat CRS consistently
        ...
        # ---- legacy/compat fields (old app.py / interaction helpers expect these) ----
        self._map_ax = ax
        ...
        # all edges as a list (some handlers expect list)
    ...
    def _geo_lookup(self, env) -> Dict[str, Tuple[float, float]]:
        ...
        """
        geo 取得を統一:
          - env.geo_lookup() があれば呼ぶ（互換）
          - ただし空(dict)なら CSV(node_geo.csv) から再取得
        """
        GEO: Dict[str, Tuple[float, float]] = {}
    ...
    # [PATCH] add data_crs arg and use it for set_extent(crs=...)
    def _auto_fit(self, ax, pos: Dict[str, Tuple[float, float]], selected_names: set[str], *, used_cartopy: bool, data_crs=None):
        ...
        if fit:
            if used_cartopy:
                try:
                    # extent is lon/lat in the data CRS (usually PlateCarree)
                    ax.set_extent([xmin_c - lon_pad, xmax_c + lon_pad, ymin - lat_pad, ymax + lat_pad], crs=data_crs)
                except Exception:
                    pass
            else:
    ...
    def _auto_fit(self, ax, pos: Dict[str, Tuple[float, float]], selected_names: set[str], *, used_cartopy: bool, data_crs=None):
        ...
        fit_keys = [k for k in selected_names if k in pos] or list(pos.keys())
        xs = [pos[k][0] for k in fit_keys]  # lon
        ys = [pos[k][1] for k in fit_keys]  # lat

        ymin, ymax = min(ys), max(ys)
        xs_wrapped = [_wrap_lon(x, center=180.0) for x in xs]
        xmin_c, xmax_c = min(xs_wrapped), max(xs_wrapped)

        lon_span = max(1e-6, (xmax_c - xmin_c))
        lat_span = max(1e-6, (ymax - ymin))

        # 初期パディング
        lon_pad = max(5.0, lon_span * 0.08)
        lat_pad = max(3.0, lat_span * 0.10)

        # lon が広すぎる場合は global 表示に倒す（帯状つぶれ回避）
        # ※ ここは好みで 180/200/220 を調整してOK
        if used_cartopy and lon_span > 200.0:
            try:
                ax.set_global()
            except Exception:
                pass
        ...
        # ---- アスペクト補正（重要）----
        # 横幅が広いのに縦が狭いと「帯」になるので、必要な lat_span を確保する
        if used_cartopy:
            try:
            ...
            # set_extent の lon/lat は「度」なので、単純に lon_span/aspect を目安に縦幅を合わせる
            target_lat_span = (lon_span + 2 * lon_pad) / max(0.5, aspect)
            current_lat_span = (lat_span + 2 * lat_pad)

            if current_lat_span < target_lat_span:
                extra = (target_lat_span - current_lat_span) / 2.0
                lat_pad += extra

        if fit:
            if used_cartopy:
                try:
                    extent = [xmin_c - lon_pad, xmax_c + lon_pad, ymin - lat_pad, ymax + lat_pad]
                    # 緯度クリップ（安全策）
                    extent[2] = max(-89.5, extent[2])
                    extent[3] = min(89.5, extent[3])
    ...
    def _auto_fit(self, ax, pos: Dict[str, Tuple[float, float]], selected_names: set[str], *, used_cartopy: bool, data_crs=None):
        ...
        # ---- 0..360 に正規化して「最小幅の経度区間」を作る ----
        lons360 = sorted(((lon % 360.0) + 360.0) % 360.0 for lon in lons)

        if len(lons360) == 1:
            lon_min360 = lons360[0]
            lon_max360 = lons360[0]
        else:
            # 最大ギャップを探し、その“反対側”が最小区間
            gaps = []
            for i in range(len(lons360) - 1):
                gaps.append((lons360[i+1] - lons360[i], i))
            gaps.append(((lons360[0] + 360.0) - lons360[-1], len(lons360) - 1))  # wrap gap
        ...
        # lon が極端に広い場合は global に倒す（帯状つぶれ回避）
        ...
        if used_cartopy:
            try:
                # ★重要：0..360（または360超）を含む lon でも cartopy は扱える
                ax.set_extent([xmin, xmax, ymin, ymax], crs=data_crs)
            except Exception:
                pass
        else:
            # non-cartopy の場合は -180..180 に寄せる
            def to180(v):
                v = ((v + 180.0) % 360.0) - 180.0
                return v
            ax.set_xlim(to180(xmin), to180(xmax))
    ...
    def _install_map_interactions(self):
        ...
        """
        # [PATCH] prefer state.canvas if legacy attr isn't set (TkAgg wrapper etc.)
