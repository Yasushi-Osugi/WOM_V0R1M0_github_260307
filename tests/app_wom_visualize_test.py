...
def _lot_color(lot: LotViewModel) -> str:
    if lot.age_weeks >= 4:
        return "#d62728"   # aged / risky
    if lot.age_weeks >= 2:
        return "#ff7f0e"   # warning
    return "#111111"       # normal
...
def build_bottleneck_summary(snapshot: dict) -> html.Div:
    ...
    top_node_1 = ranked_nodes[0] if len(ranked_nodes) > 0 else ("N/A", 0.0)
    top_node_2 = ranked_nodes[1] if len(ranked_nodes) > 1 else ("N/A", 0.0)
    top_edge_1 = ranked_edges[0] if len(ranked_edges) > 0 else ("N/A", 0.0)
        ...
        if lot:
            sections.extend(
                [
                    ...
                    html.P(f"age_risk_color: {_lot_color(lot)}"),
